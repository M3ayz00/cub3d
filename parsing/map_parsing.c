...
/*   Updated: 2024/09/24 22:27:06 by msaadidi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_map_valid(t_lst **rows)
{
	...
	while (curr)
	{
		...
		{
			if (check_next_lines(curr) && check_previous_lines(curr))
			{
...
int	check_line(char **line, t_cub3d *cub3d, int *i)
{
	if (is_identifier(*line) == 1)
	{
		...
		return (free(cub3d->container), free(*line),
			free_map(&cub3d->map), free_textures(&cub3d->textures), 0);
	else
		free(*line);
	return (1);
}
...
int	parse_cub3d(t_cub3d *cub3d, char *path, int is_bonus)
{
	...
	if (!check_files(cub3d))
		return (close(fd), free_map(&cub3d->map),
			free_textures(&cub3d->textures), 0);
	return (close(fd), 1);
}
